...
class ExampleApp:
    def __init__(self):
        self.window = LingmoPyUI.LingmoWindow(
            showDark=True, showStayTop=True, width=500, height=500, title="Example"
        )
    ...
    def setup_ui(self):
        ...
        self.scrollbar = LingmoPyUI.LingmoScrollBar(
            self.window.contentItem, target=self.frame, autoOffsetIncrease=True
        )
        ...
        self.progressButton = LingmoPyUI.LingmoProgressButton(
            self.frame, content="Progress", progress=0.5
        )
        ...
        self.progressRing2 = LingmoPyUI.LingmoProgressRing(
            self.frame, progressVisible=True, progress=0.5
        )
        ...
        self.menuItem2 = LingmoPyUI.LingmoMenuItem(
            iconSource=LingmoPyUI.LingmoIconDef.Admin, content="Icon MenuItem"
        )
        self.menu.addItem(self.menuItem2)
        self.subMenu = LingmoPyUI.LingmoMenu()
        self.menuItem3 = LingmoPyUI.LingmoMenuItem(
            content="SubMenu MenuItem", subMenu=self.subMenu
        )
        self.menu.addItem(self.menuItem3)
        self.menuItem4 = LingmoPyUI.LingmoMenuItem(
            content="Checkable MenuItem", checkable=True
        )
        ...
        a = 0
        self.dropDownBox = LingmoPyUI.LingmoDropDownBox(
            self.frame, content="DropDownBox"
        )
        ...
        self.filledButton.pressed.connect(
            lambda: self.window.showWarning("This is all the widgets", 1500, "OK")
        )
        self.loadingButton.pressed.connect(self.window.showLoading)
        self.iconButton.pressed.connect(
            lambda: self.window.showInfo("This is all the widgets", 1500, "OK")
        )
