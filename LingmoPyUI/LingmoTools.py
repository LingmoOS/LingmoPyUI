...
def hash_compile_time(s,last_value=basis):
    return hash_compile_time(s[1:],(s[0]^last_value)*prime) if s!='' else last_value
...
def md5(text:str):
    return QCryptographicHash.hash(text.encode('utf-8'),QCryptographicHash.Algorithm.Md5).toHex()
...
def sha256(text: str):
    return QCryptographicHash.hash(text.encode('utf-8'),QCryptographicHash.Algorithm.Sha256).toHex()
...
def windowBuildNumber():
    if isWin():
        regKey=QSettings(R'(HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion)',QSettings.Format.NativeFormat)
...
def getWallpaperFilePath():
    if isWin():
        path=[]
        if user32.SystemParametersInfoW(SPI_GETDESKWALLPAPER,wintypes.MAX_PATH,path,False)==False:
            return []
        return path
    elif isLinux():
        typeSys=QSysInfo.productType()
        if hash_(typeSys)==hash_compile_time('uos'):
            interface=QDBusInterface('com.deepin.wm','/com/deepin/wm','com.deepin.wm',QDBusConnection.sessionBus())
            if not interface.isValid():
                qWarning(QDBusConnection.sessionBus().lastError().message())
                return ''
            reply=interface.call('GetCurrentWorkspaceBackgroundForMonitor',"string:'"+currentTimestamp()+"'")
            if reply.isValid():
            ...
            interface=QDBusInterface('com.lingmo.Settings','/Theme','org.freedesktop.DBus.Properties',QDBusConnection.sessionBus())
            if not interface.isValid:
                qWarning(QDBusConnection.sessionBus().lastError().message())
                return ''
        ...
        process=QProcess()
        process.start('osascript','-e(tell application "Finder" to get POSIX path of (desktop picture as alias))')
    ...
    for i in range(image.width()):
        for j in range(image.height()):
            if image.valid(i,j):
                ...
                g+=c.green()
                b+=c.blue()
    return QColor(min(int(bright*r/t),255),min(int(bright*g/t),255),min(int(bright*b/t),255))
